...
SYSTEM_PROMPT = get_system_prompt_for_agent("confeiteiro")

# 2. Inicialização do Modelo com Ferramentas
# O Confeiteiro precisa ler as notas e controlar o forno
tools = [control_oven, read_notes]
model = get_model().bind_tools(tools)

def call_confeiteiro(state: AgentState, config: RunnableConfig) -> AgentState:
    """
    Nó do Confeiteiro: Lê a tigela, consulta o caderno e executa a transformação.
    """
