...
O ten é um agente pessoal de Inteligência Artificial para operar 100% localmente no desktop do usuário. Ele recebe comandos exclusivamente pelo Telegram, processa-os através de um pipeline que suporta múltiplos LLMs dinamicamente, e tem acesso persistente à memória em SQLite.
...
- [ ] G-03: Reter contexto por múltiplos turnos com SQLite via repositórios Python.
...
O usuário envia um chat no Telegram, o ten roda local em background num terminal, chama LLMs, lê Skills em pastas locais, aciona ferramentas e responde no mesmo chat de forma orgânica.
...
|----|-----------|-----------|-------------------|
| RF-01 | O sistema deve rodar via loop de polling persistente da biblioteca aiogram | Must | O terminal aciona o listener com `uv run main.py` e intercepta as mensagens sem fechar. |
...
| RNF-02 | Persistência Ágil | SQlite Assíncrono | `aiosqlite` escolhido pela performance e simplicidade no loop assíncrono do Python |
...
|-------------|------|------------------------|
| Telegram API | Obrigatória | O agente se tornará inutilizável / Modo sleep no Python. |
| APIs (Gemini/DeepSeek) | Obrigatória | Sem raciocínio lógico. Precisará tentar fallback no `ProviderFactory`. |
| pacote `aiogram` | Obrigatória | Lib python core da arquitetura de polling |
...
- **Estratégia:** Deploy em máquina local rodando `uv run main.py` para a instância primaria, utilizando o `uv` para gestão otimizada e rápida das dependências.
