...
**Por que agora:**
Precisamos desacoplar a parte de aiogram/entrada de dados da parte de processamento puramente sistêmico (Tool calls/Registry).
...
- NG-02: Executar Tools de forma paralela usando workers (as tool calls serão tratadas resolutivamente em cascata via iterador assíncrono sequencial no escopo da mesma iteração ReAct).
...
|----|-----------|-----------|------------|
| RNF-01 | Timeout por interção unitária LLM | < 120s | Pra evitar socket timeout/hang do Python |
...
| EC-02: Ferramenta retorna Throw (Error hard) | Tentou criar num path que não existe na máquina host do Python (`FileNotFoundError`). | O except manda o log de erro do trace como string de observação de volta pra IA corrigir caminho. |
