...
| Filesystem nativo Python | Obrigatória | Skill Loader System paralisa em exception de path inexistente `FileNotFoundError`. |
...
- Autenticação e Autorização estão em camada externa (Bot aiogram Handler), as skills operam blindly sem verificar Whitelist - assumindo permissão plena concedida globalmente ao AgentController via Telegram UserId check.
