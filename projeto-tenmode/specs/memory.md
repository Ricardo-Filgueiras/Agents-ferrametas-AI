...
O módulo de persistência de estado do ten gerencia tanto as conversas de longo prazo em banco de dados SQLite (`aiosqlite`) quanto atua como manager da janela de contexto para impedir que o limite maximo do envelope de tokens da IA (Context Window) estoure.
...
**Evidências:**
Tentativas de armazenar arrays in-memory no Python funcionam apenas até o app ser encerrado/reiniciado. O histórico vaporizava.
...
- NG-02: ORMs complexos como SQLAlchemy. Usaremos SQL via `aiosqlite` por leveza e clareza. 
...
|----|-----------|-----------|------------|
| RNF-01 | Transações Seguras | Auto-commit nativo | WAL ativo resolve concorrência e bloqueios no Python. |
...
Sem foreign keys restritas ativadas com pragma foreign_keys=ON pra priorizar inserts mais leves sem checks, apenas referências programáticas via código Python.
...
|-------------|------|------------------------|
| `aiosqlite` | Obrigatória | O agente vai originar erro na inicialização (ex: `main.py`). |
| Filesystem (`os`/`pathlib`) | Obrigatório | DB path tem que ser gerido e gravado via permissões nativas. |
...
- **Arquivos DB Sensíveis:** O `ten.db` jamais pode ir pro Git (Adicionar no `.gitignore` /data).
