...
O módulo Telegram Input recebe eventos brutos advindos das APIs do Telegram via biblioteca aiogram (Long Polling), faz a filtragem de segurança por whitelist de ID, converte anexos (documentos PDF e arquivos/mensagens de voz de áudio) em texto viável, e injeta na memória do ciclo de agente para resolução AI.
...
**Por que agora:**
A lib aiogram suporta download de anexo. O Python usa a ponte em RAM / disco temporário para extração usando o PyPDF2 ou pdfplumber, e a integração com um modelo Whisper local permite extrair o texto de qualquer áudio enviado no chat de forma privada e sem custos de API externa.
...
2. Bot aiogram valida se user ID = `TELEGRAM_ALLOWED_USER_IDS`.
...
|-------------|------|------------------------|
| aiogram / python-telegram-bot | Obrigatória | Nenhuma intercepção ocorrerá. |
| Lib PDF (PyPDF2, pdfplumber) | Secundária | Texto cairá no Agent Loop como string vazia de documento ininteligível. |
...
| EC-02: OOM (Out of Memory) no Whisper | Áudio massivo pesa e Whisper crasha o processo no host. | Timeout de 60s e bloco except do Python envelopa falha. O sistema envia e o usuário recebe: "⚠️ Falha ao processar o áudio: arquivo grande demais ou falha no serviço." |
...
- Como acionar o Whisper local a partir do backend Python? *Decisão pendente: usar subprocess nativo ou lib whisper python-bindings vinculados ao Torch.*
