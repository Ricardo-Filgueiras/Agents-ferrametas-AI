...
| RF-02 | O FileOutputStrategy deve interceptar a flag de envio em arquivo de Markdown e salvar temporariamente no Python pra usar Upload de buffer aiogram `reply_document()`. | Must | Geração de documento na Skill envia um anexo com Titulo formatado apropriadamente. |
| RF-03 | O ErrorOutputStrategy formata em bloco emoji amarelo e dispara apenas avisos. | Must | Erros críticos de Prompt/API disparam "⚠️ Erro: X" ao invés de quebra silenciosa no Python. |
...
|----|-----------|-----------|------------|
| RNF-01 | Ordem das Mensagens (Sync) | 100% Cronologico | Chunks mal organizados perdem sentido. Async/Await estrito no "for" loop em vez de gather concorrente de async promises. |
...
|-------------|------|------------------------|
| API aiogram (Send) | Obrigatória | Nenhuma response vai chegar. Timeout nativo por Retry do app telegram após 60 segs (loop eterno para o server python). |
...
|---------|---------|----------------------|
| EC-01: Rate Limiting Telegram (429 Too Many Requests) | O Output gerou 30 chunks sequenciais por um arquivo colossal e o TG bloqueia Flood. | A api deve emitir Erro 429. Catch Strategy dorme a coroutine do Python (`asyncio.sleep`) exposto pelo cabeçalho `Retry-After` da API, e re-dispara via fila com buffer na sequencia restrita sem perda do payload do chunk. |
...
| EC-03: Bot Blocker | O user bloqueia o bot ou desativa mid-reply. | O `bot.send_message` lança erro "Forbidden". Output handler descarta em Except e loga "Msg abandonada, User bot-blocked" para não falhar stack Python. |
